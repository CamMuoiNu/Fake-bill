<?php 
    ...
    if($_POST['type'] == 'Login' )
    {
        ...
        if(!$NMQ->get_row(" SELECT * FROM `users` WHERE `username` = '$username' "))
        {
        ...
        if($NMQ->get_row(" SELECT * FROM `users` WHERE `username` = '$username' AND `banned` = '1' "))
        {
            msg_error2('Tài khoản này đã bị khóa bởi BQT');
        }
        if(!$NMQ->get_row(" SELECT * FROM `users` WHERE `username` = '$username' AND `password` = '$password' "))
        {
    ...
    if($_POST['type'] == 'Register' )
    {
        ...
        if($NMQ->num_rows(" SELECT * FROM `users` WHERE `ip` = '".myip()."' ") > 3)
        {
            msg_error2('Bạn đã đạt giới hạn tạo tài khoản');
        }
        $create = $NMQ->insert("users", [
            'username'      => $username,
    ...
    if($_POST['type'] == 'ChangePassword')
    {
        ...
        $row = $NMQ->get_row(" SELECT * FROM `users` WHERE `otp` = '$otp' ");
        if(!$row)
        {
    ...
    if($_POST['type'] == 'DoiMatKhau')
    {
        ...
        $row = $NMQ->get_row(" SELECT * FROM `users` WHERE `username` = '".$_SESSION['username']."' ");
        if(!$row)
        {
