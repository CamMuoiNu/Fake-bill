<?php
    ...
    $title = 'CHỈNH SỬA THÀNH VIÊN | '.$NMQ->site('tenweb');
    require_once("../../public/admin/Header.php");
    require_once("../../public/admin/Sidebar.php");
?>
<?php
/* BẢN QUYỀN THUỘC VỀ NMQ.CO | NTTHANH LEADER NT TEAM */
if(isset($_GET['id']) && $getUser['level'] == 'admin')
{
    $row = $NMQ->get_row(" SELECT * FROM `users` WHERE `id` = '".check_string($_GET['id'])."'  ");
    if(!$row)
    {
...
if(isset($_POST['btnCongTien']) && isset($_POST['value']) && isset($row['username']) && $getUser['level'] == 'admin')
{
    ...
    if($create)
    {
        $NMQ->cong("users", "money", $value, " `username` = '".$row['username']."' ");
...
if(isset($_POST['btnTruTien']) && isset($_POST['value']) && isset($row['username']) && $getUser['level'] == 'admin')
{
    ...
    if($create)
    {
        $NMQ->tru("users", "money", $value, " `username` = '".$row['username']."' ");
                        ...
                        <div class="table-responsive">
                            <table id="datatable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($NMQ->get_list(" SELECT * FROM `dongtien` WHERE `username` = '".$row['username']."' ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
